...
@Component
public class DnaHumanoDAOImpl implements DnaDAO {
	...
	public Long count() throws DBException {
		...
		try{
			session = config.getCurrentSession();
			session.beginTransaction();
			count = (Long) session.createCriteria("com.matias.model.impl.DnaHumano").setProjection(Projections.rowCount())
					.setCacheable(true).setCacheRegion("HUMAN_COUNT_CACHE").uniqueResult();
			session.getTransaction().commit();
			return count;
		} catch(Exception sqlException) {
			if(null != session.getTransaction()) {
